...
namespace SwarmECS.Simulation.Spatial
{
    ...
    internal static class SpatialQuerySort
    {
        ...
        public static void Sort(ulong[] distances, int[] entityIds, int count)
        {
            if (count < 2)
            {
                return;
        ...
        private static void SiftDown(ulong[] distances, int[] entityIds, int root, int count)
        {
            while (true)
            {
                int child = (root << 1) + 1;
        ...
        private static bool IsGreater(ulong leftDistance, int leftId, ulong rightDistance, int rightId)
        {
            return leftDistance > rightDistance || (leftDistance == rightDistance && leftId > rightId);
        }
        ...
        private static void Swap(ulong[] distances, int[] entityIds, int left, int right)
        {
            ulong distance = distances[left];
            distances[left] = distances[right];
            distances[right] = distance;
