...
本工程把商业管线做成独立边界，核心定点数仿真不依赖资源系统或热更运行时。这样既能展示真实接入，又不会让算法测试被打包工具污染。
...
仓库不会提交 `HybridCLRData/LocalIl2CppData-*`、平台构建产物或 CDN 包；这些应由受控构建机生成。算法 Demo 在没有下载任何热更包时也能独立运行。
